...
public:
	...
	wxString     GetInternalName() const wxOVERRIDE
	{
		return m_Name;
	}
	wxPathFormat GetInternalFormat() const wxOVERRIDE
	{
		return wxPATH_UNIX;
	}
	int          GetMode() const;
	...
	wxFileOffset GetSize() const wxOVERRIDE
	{
		return m_Size;
	}
	wxFileOffset GetOffset() const wxOVERRIDE
	{
		return m_Offset;
	}
	wxDateTime   GetDateTime() const wxOVERRIDE
	{
		return m_ModifyTime;
	}
	...
	bool IsReadOnly() const wxOVERRIDE
	{
		return !(m_Mode & 0222);
	}
...
private:
	...
	virtual wxArchiveEntry* DoClone() const wxOVERRIDE
	{
		return Clone();
	}
